...
add_env() {
	local name="$1"
	local value="$2"
	printf '%s' "$value" | vercel env add "$name" development >/dev/null
	echo "Added $name"
}

add_env NEXT_PUBLIC_API_URL http://localhost:3000/api
...
add_env JWT_SECRET your_jwt_secret_key_here_change_in_production
add_env API_KEY your_api_key_here_change_in_production
add_env SECRET_KEY your_secret_key_here_change_in_production
add_env CORS_ORIGIN http://localhost:3000
add_env ALLOWED_ORIGINS localhost:3000,127.0.0.1:3000
...
add_env DEBUG_MODE true
add_env HOT_RELOAD true
add_env STRICT_MODE false
add_env EXPOSE_ERROR_DETAILS true

echo "All development environment variables were added to Vercel."